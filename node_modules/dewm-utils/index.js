...
utils.addStacks = function(stacks) {
	...
	db.collection('stacks',function(err,collection) {
		...
		bulk.execute(function(err,result) {
			...
			else { console.log("Sucessfully inserted "+result.nUpserted+" stacks and updated "+result.nModified+" stacks for the "+returnStacks[0].issue+" issue in MongoDB.") } 
		});    
    });
	return returnStacks;
}
...
utils.addIssues = function(toAdd) {
	if (toAdd.length>0) {
		db.collection('issues').insert( toAdd, function(err, result) {
			if (err) { console.log(err) };
			if (result) { console.log('Sucessfully inserted some issues into MongoDB.'); };
		});		
	}
}
