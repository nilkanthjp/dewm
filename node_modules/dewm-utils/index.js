			...
			bulk.execute(function(err,result) {
				if (err) { console.log(err) } 
					...
					if (parseInt(result.nUpserted)>0) {
						...
						console.log("Added "+result.nUpserted+" "+database+"s to MongoDB.")
					}
	...
	db.collection("stacks",function(err,collection) {
		...
		for (var stack in actions) {
			...
			if (action === false) {
				...
				var date=utils.dateFromStack(stack),
		...
		bulk.execute(function(err,result) {
			...
			if ( Object.keys(change).length>0 ) { utils.updateStacks(change); }
		});
    });
}
...
utils.updateStacks = function(changes) {
