...
utils.mongo.add = function(database,documents,field,property) {
	...
	if (documents.length>0) {
		db.collection(database,function(err,collection) {
			...
			bulk.execute(function(err,result) {
				...
				else { console.log("Inserted "+result.nUpserted+" "+database+" and updated "+result.nModified+" "+database+" in MongoDB.") } 
			});    
	    });	
	}
}
...
utils.mongo.find = function(database,query,callback) {
	db.collection(database).find( { $or: query }).toArray(function (err, items) {
		callback(err,items);
	});
}
