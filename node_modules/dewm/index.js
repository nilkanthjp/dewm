		...
		if (data===undefined) {
			...
			xml.parseString(data, function (err, result) {
				if (result == undefined) {
					...
					var response = "",
						stacks = self.addStacks(result.sidecar.entry);
...
dewm.monitorComments = function(stack) {
	...
	var	commentListener = oplog(paths.mongo.host+"local", 'dewm.comments').tail();
	commentListener.on('op', function (data) {
		utils.comments(function(items) {
			commentEvent.emit("newComment",items);
			if (data.op=="i") {
...
dewm.addStacks = function(stacks) {
	...
	utils.mongo.add("stacks",returnStacks,"stack","stack",function(added){
		utils.mongo.find("stacks",{issue:issue,stack:{$nin:stackNumbers}},function(err, items){
			if (items!==null) {
				var deleted = [];
				for(var i=0; i<items.length; i++){
					deleted.push(items[i].stack);
				}
...
dewm.initSocket = function (server) {
    ...
    io.on('connection', function (socket) {
        socket.on('reqComment', function (data) {
            ...
            commentEvent.on("newComment",function(comments){
