...
dewm.getDates = function() {
	...
	fs.readdir( paths.greenRoot, function (err, files) { 
	    ...
	    self.dates = dates;
	    self.weeks = [];
	    for (var i=0; i<dates.strings.length; i++) { self.weeks.push({"files":[],"stacks":[],"monitored":false}) }
			...
			if (data != undefined) {
				xml.parseString(data, function (err, result) {
					if (result == undefined) {
						...
						if (self.weeks[week].monitored==false) { self.monitorWeek(week); };
						var response = "";
					}
					if(callback) {
						callback(response);
...
dewm.monitorWeek = function(week) {
	var self = this,
		sidecar = self.weeks[week].pathOrange+"/Sidecar/sidecar.xml";
