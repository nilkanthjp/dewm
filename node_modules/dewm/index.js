...
dewm.getDates = function() {
	...
	fs.readdir( paths.greenRoot, function (err, files) { 
		...
		self.setWeek(dates.strings.length-6);
	});
};
...
dewm.getWeek = function(week,callback) {
	...
	fs.readFile(self.weeks[week].sidecar, "utf8", function (err, data) {
		if (data===undefined) { self.setWeek(week-1); }
		else {
			xml.parseString(data, function (err, result) {
				if (result == undefined) {
					...
					var response = "", stacks = self.formatStacks(result.sidecar.entry);
					self.weeks[week].files = stacks.files;
					self.weeks[week].stacks = stacks.stacks;
					//build.gallery.init(10,14,"tablet",self);
				};
...
dewm.monitorWeek = function(week) {
	...
	setInterval(function() {
		fs.stat(self.weeks[week].sidecar,function(err, stats){
			...
			if (self.weeks[week].modified<modified) { console.log("Update to the sidecar for "+dewm.dates.strings[week]); self.getWeek(week);
			};
		});
	},15000);
};
...
dewm.initSocket = function (server,session) {
    ...
    io.on('connection', function (socket) {
    	var newStack = function(stacks){ socket.emit('newStack', stacks); };
    	var newAssignment = function(stack){ socket.emit('assignments', stack); };
        ...
        socket.on('reqComment', function (data) {
        	utils.comments(function(comments){ socket.emit('newComment',comments); },data.stack,data.dept);
		...
		socket.on('reading', function (data) { utils.checkReader(data.stack,data.username,true); });
    });	
}
