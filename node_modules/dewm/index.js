...
dewm.monitorComments = function(stack) {
	...
	var	commentListener = MongoOplog(paths.mongo.host+"local", 'dewm.comments').tail();
...
dewm.checkMissingIssues = function(existing,dates) {
	var self = this,
		mongoDates = [],
		mongoToAdd = [];
	utils.mongo.find("issues",existing,function(err, items){
		for (var i=0; i<items.length; i++) {
...
dewm.addStacks = function(stacks) {
	...
	utils.mongo.add("stacks",returnStacks,"stack","stack");
        ...
        socket.on('reqComment', function (data) {
            commentEvent.on("newComment",function(comments){
	            ...
	            for (var i=0; i<comments.length; i++) {
		            if (comments[i].stack==data.stack && comments[i].type==data.type) {
