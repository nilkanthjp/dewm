...
dewm.getDates = function() {
	...
	var unordered = [];
	fs.readdir( paths.tabletGreenRoot, function (err, files) { 
	    if (!err) {
		...
		self.getWeek(dates.strings.length-1);
	});
};
...
dewm.getWeek = function(week,mode,callback) {
	...
	fs.readdir( paths.tabletGreenRoot+issueDate+"/TNY"+issueDate+"_SourceFiles/TNY"+issueDate+"_MasterSource/", function (err, files) { 
		self.weeks[week] = {"files":files};
		var isThisWeekReady = true;
		if (files && files.length<2) {
			isThisWeekReady = false; 
		}
		fs.readFile(paths.tabletOrange+mode+issueDateWithHyphens+"/Sidecar/sidecar.xml", "utf8", function (err, data) {
			if (data != undefined) {
				xml.parseString(data, function (err, result) {
					if (result == undefined) {
						self.current = self.dates.strings.length-1;
						var response = "The Sidecar XML for "+issueDateFormatted+" is malformed and so it cannot be loaded. Please fix it and try again.";
					} else {
						self.weeks[week].stacks = result.sidecar.entry;
						for (var i=0; i<self.weeks[week].stacks.length; i++) {
							var title = self.weeks[week].stacks[i].articleTitle[0];
							self.weeks[week].stacks[i].articleTitle = title.split(" by ")[0];
							self.weeks[week].stacks[i].author = title.split(" by ")[1];
						}
						var response = "";
					}
					if(callback) {
