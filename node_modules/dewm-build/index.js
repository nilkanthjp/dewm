...
var build = {xml:{},html:{},touts:{},elements:{}};
...
build.init = function(w,i,type,dewm) {
	...
	fs.readFile(xmlfile, "utf8", function (err, xml) {
		...
		fs.exists(output+stack, function (exists) {
			if (!exists) { fs.mkdir(output+stack,function(err,data){ build.xml.transform(stack,xml,type,htmlfile); });} 
			else { build.xml.transform(stack,xml,type,htmlfile); };
		});
	});
}
...
build.xml.transform = function(stack,xml,type,htmlfile) {
	if (stack.indexOf("0100_") == 0) { var transform = build.toc; }
	else { var transform = build.default; };
	transform(xml,function(html) {
		if (type=="danville") { html = html.replace("<!--insert danville script here-->",templates.danville.scripts); };
		fs.writeFile(htmlfile, html);
	});
};

build.xml.pre = function(xml) {
	// Remove the ampersand from the Twitter share link that causes XML to be malformed.
	xml=xml.replace(/app&body=I/g, "app+body=I");
	// Escape certain fields in the XML that would be parsed incorrectly by xml2js.
	xml=build.xml.escape(xml,["elementcaption","tout_1","tout_2","tout_3","tout_4","tout_5"]);
	return xml;
};
...
build.xml.post = function(xml,content) {
	xml=xml.document,
	xml.content=content;
	xml.shareurls.share_email=xml.shareurls.share_email.replace("app+body=I","app&body=I");
...
build.default = function(xml,callback) {
	...
	xml2js.parseString(xml, {explicitArray: false}, function (err, result) {
		if (err) { console.log("The XML for the stack is malformed.")}
			...
			xml=build.xml.post(result,content);
			transformations.router(xml.stackname)(xml,function(html) {callback(html);});
		};
	});
};
...
build.elements.init = function(xml) {
	var elements = xml.articleelements.element,
		output=[], date=utils.format.date.stack(xml.stackname);;
	if (elements) {
		if (elements.length===undefined) { elements=[elements]; }
		for (i in elements) {
			var element=elements[i],
				type=element.type.split("_")[1];
			output.push(build.elements[type](element,i,xml.stackname,date))
		};
	};
	return output;
};

build.elements.cartoon = function(element,i,stack,date) {
...
build.elements.spot = function(element,i,stack,date) {
...
build.elements.art = function(element,i,stack,date) {
...
build.elements.newsbreak = function(element,i,stack,date) {
	var file = element.imagename+".xml";
...
var generateElementsObject = function(elementsXML,danvilleImage,stackname,rootDirectory) {
...
var copyImage = function(imagename,stackName,size,rootDirectory) {
