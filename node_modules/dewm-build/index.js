...
var build = {xml:{},html:{},touts:{},elements:{},image:{},menu:{cartoon:{},toc:{}}};
...
build.menu.toc.init = function(xml,output,callback) {
	...
	xml2js.parseString(xml, {explicitArray: false}, function (err, xml) {
		var date = utils.format.date.stack(xml.document.stackname), week = dewm.dates.strings.indexOf(date), 
			toc = {dept:"",crit:"",cover:"", date:dewm.dates.dates[week], dateString:date, dateFormatted:dewm.dates.stringsFormatted[week], volume:parseInt(xml.document.volumenumber), issue:parseInt(xml.document.issuenumber)}, 
			stacks = build.menu.toc.stacks(dewm.weeks[week].files,week), html = templates.stacks.cartoon;
		build.menu.toc.build(stacks,0,toc,function(cartoon) {
			html = transformations.toc.body(toc,date);
			callback(html,xml,response);
		});
	});
};

build.menu.toc.stacks = function(stacks,week) {
	var candidates=[];
	for (var i=0; i<stacks.length; i++) {
...
build.menu.toc.check = function(xmlfilename,toc,callback) {
	fs.readFile(xmlfilename, "utf8", function (err, xml) {
		if (xml!==undefined) {
			xml=build.xml.pre(xml);
			xml2js.parseString(xml, {explicitArray: false}, function (err, xml) {
				xml=xml.document;
				toc=transformations.toc.router(xml.stackname)(xml,toc);
				callback(toc);
			});
		} else { callback(toc); }
	});
...
build.menu.toc.build = function(stacks,i,toc,callback) {
	if (i<stacks.length) {
		build.menu.toc.check(stacks[i].xmlfilename,toc,function(toc) { build.menu.toc.build(stacks,i+1,toc,callback); }); 
	} else { callback(toc) };
};
...
build.image.art = function(xml,to) {
	...
	if (xml.image) {
		...
		files = [ {from:paths.poem, type:"thumbnail", to:to+xml.stackname+"/", filename:"toc.png"} ];
	};
