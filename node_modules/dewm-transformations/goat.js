...
function transformGoatTables(theCondeNetXMLText,stackName,template,location,rootDirectory) {
	...
	var tablesInfo = danvilleHTML.split('<p><i>')[danvilleHTML.split('</i></p>').length-1];
	danvilleHTML = danvilleHTML.split('<p><i>')[0];
	...
	if (stackName.indexOf("tables") > 0) {
		...
		if (typeof xmlBarName != "undefined") {
			danvilleHTML = danvilleHTML.replace(new RegExp("<!--insert bartab here-->", 'g'), '<a href="navto://'+xmlBarName+'">Bar Tab</a>');
		}
...
function transformGoatMenu(theCondeNetXMLText,stackName,template,location,rootDirectory) {
	...
	for (var i=0; i<window.stackNames.length; i++) {
		if (window.stackNames[i].indexOf("goat") > 0 && window.stackNames[i].indexOf("opener") == -1 && window.stackNames[i].indexOf("menu") == -1 ) {
			if (window.stackNames[i].indexOf("li_"+theDate) > 0 || window.stackNames[i].indexOf("bartab") > 0) {
	...
	var goatMenuHTML = "";
	for (var w=0; w<goatMenuStacks.length; w++) {
...
function transformGoatSection(theCondeNetXMLText,stackName,template,location,rootDirectory) {
	...
	if (leadIn) {
		template = window.goatLeadInTemplate;
		var danvilleHTML = theCondeNetXMLText.slice(theCondeNetXMLText.indexOf("<content>")+10,theCondeNetXMLText.indexOf("</content>"));
		...
		if (location.Image.length > 0) {
			template = window.goatNoLeadInArtTemplate;
		} else {
...
function generateDVD(theCondeNetXMLText) {
	...
	dvdHTML = dvdHTML.replace(new RegExp("<!--insert ref here-->", 'g'), toutsXML.split("<ref_1>")[1].split("</ref_1>")[0]);
	dvdHTML = dvdHTML.replace(new RegExp("<!--insert imagename here-->", 'g'), toutsXML.split("<ref_1>")[1].split("</ref_1>")[0].split("-")[1]+"-"+toutsXML.split("<ref_1>")[1].split("</ref_1>")[0].split("-")[2]);
	try {
		dvdHTML = dvdHTML.replace(new RegExp("<!--insert title here-->", 'g'), toutsXML.split("<tout_1>")[1].split("&#8220;")[1].split("</tout_1>")[0].split("&#8221;")[0].slice(0,-1));
	} catch(err) {
		alert("Movie of the Week tout text is not in yet. It will need to be added manually.");
	}
	return dvdHTML;
}
