...
var transformations = {toc:{},cartoon:{},gallery:{}};
...
transformations.cartoon.stack = function(cartoon,date) {
	var html = templates.stacks.cartoon.replace(new RegExp('<!--insert image here-->', 'g'), cartoon.imagename).replace(new RegExp('<!--insert date here-->', 'g'), date);
	if (cartoon.elementcaption) { html=html.replace(new RegExp('<!--insert caption here-->', 'g'), unescape(cartoon.elementcaption)); }
	return html;
};

transformations.toc.body = function(toc) {
	return templates.toc.body.replace(new RegExp("<!--insert date here-->", 'g'), toc.dateString)
	.replace(new RegExp("<!--insert month here-->", 'g'), toc.date.getMonth())
	.replace(new RegExp("<!--insert year here-->", 'g'), toc.date.getFullYear())
	.replace(new RegExp("<!--insert day here-->", 'g'), toc.date)
	.replace(new RegExp("<!--insert formatted date here-->", 'g'), toc.dateFormatted)
	.replace("<!--insert talk blurb here-->", toc.talk)
	.replace("<!--insert toc depts here-->", toc.dept)
	.replace("<!--insert toc fiction here-->", toc.fiction)
	.replace("<!--insert toc crits here-->", toc.crit)
	.replace("<!--insert cover here-->", toc.cover)
	.replace("<!--insert volume number here-->", utils.format.date.roman(toc.volume))
	.replace("<!--insert issue number here-->", toc.issue);
};

transformations.toc.router = function(stack) {
	var routes = {
		"dept":{func:transformations.toc.dept,template:templates.toc.dept,prop:"dept"},
		"fiction":{func:transformations.toc.dept,template:templates.toc.fiction,prop:"fiction"},
		"crit":{func:transformations.toc.crit,template:templates.toc.dept,prop:"crit"},
		"poem":{func:transformations.toc.dept,template:templates.toc.poem,prop:"dept"},
		"shouts":{func:transformations.toc.dept,template:templates.toc.shouts,prop:"dept"},
		"talk":{func:transformations.toc.talk,template:null,prop:"talk"},
	}, section = stack.split("_")[1];
	if (routes[section]===undefined) { return {func:transformations.toc.none,template:null,prop:null}; }
	else if (section==="crit" && stack.indexOf("_briefly_")>-1) { return {func:transformations.toc.dept,template:templates.toc.briefly,prop:"crit"}; }
	else { return routes[section]; }
};

transformations.toc.touts = function(xml) {
 	var extras = xml.tabletmultimediaextras, touts="";
	if (extras.tabletextra_1.type_1) { touts=touts+"\t\t\t\t\t\t<h6>Plus: Multimedia tout TK.</h6>\r"; }
	for (var i=1; i<6; i++) {
		if (extras["tabletextra_"+i]["type_"+i]) { 
			touts=touts+templates.toc.tout.replace("<!--insert tout type here-->",extras["tabletextra_"+i]["type_"+i].toLowerCase().replace(/ /g,"")); 
		};	
	};
	return touts;
};

transformations.toc.none = function(xml,toc,template,section) {
	return toc;
};

transformations.toc.talk = function(xml,toc,template,section) {
	if (xml.stackname.indexOf("_comm_")>-1) { toc.talk+=xml.author+" on blurb tk; "; }
	else if (xml.stackname.indexOf("_surowiecki_")>-1) { toc.talk+=xml.author+" on blurb tk."; }
	else { toc.talk+="blurb tk; "; };
	return toc;
};

transformations.toc.dept = function(xml,toc,template,section) {
	var touts = transformations.toc.touts(xml);
	toc[section] += template.replace(new RegExp("<!--insert stackname here-->", 'g'), xml.stackname)
	.replace(new RegExp("Rubric TK", 'g'), xml.title)
	.replace(new RegExp("Author TK", 'g'), xml.author)
	.replace(new RegExp("Title TK", 'g'), xml.title)
	.replace(new RegExp("<!--insert multimedia here-->", 'g'), touts);
	return toc;
};

transformations.toc.crit = function(xml,toc,template,section) {
	var touts = transformations.toc.touts(xml);
	toc[section] += template.replace(new RegExp("<!--insert stackname here-->", 'g'), xml.stackname)
	.replace(new RegExp("Rubric TK", 'g'), xml.subhead)
	.replace(new RegExp("Author TK", 'g'), xml.author)
	.replace(new RegExp("Title TK", 'g'), xml.title)
	.replace(new RegExp("<!--insert multimedia here-->", 'g'), touts);
	return toc;
...
transformations.toc.cover = function(stack,toc) {
	toc.cover = templates.toc.cover.replace(new RegExp("<!--insert stackname here-->", 'g'), stack.stack)
	.replace(new RegExp("Rubric TK", 'g'), stack.kicker)
	.replace(new RegExp("Author TK", 'g'), stack.author)
	.replace(new RegExp("Title TK", 'g'), stack.title);
	return toc;
};

transformations.gallery.router = function(stack) {
	return transformations.gallery.default;
};

transformations.gallery.default = function(xml,callback) {
	var js = templates.gallery.json, response = {status:true,text:"Successfully built "+xml.stackname+" gallery."},
		images = [], date = utils.format.date.stack(xml.stackname);
	if(xml.image) {
		var json = "";
		images.push(transformations.gallery.image(date,xml.image,xml.caption,xml.gallerycredit));
		for (var i=0; i<images.length; i++) { json+=images[i].js; };
		js = js.replace("<!--insert images here-->",json);
	} else { response.status=false; response.text="Sorry, there is no art to make a gallery for the "+xml.stackname+" stack."; };
	callback(js,images,response);	
};

transformations.gallery.image = function(date,r,caption,credit) {
	var split = utils.format.date.split(date), 
		src = paths.images.npp.replace("<!--year-->","20"+split.y).replace("<!--month-->",split.m).replace("<!--day-->",split.d)+"2560/"+date+"_"+r+".jpg",
		dimensions = size(src),
		js = templates.gallery.image.replace("<!--insert caption here-->",caption)
		.replace("<!--insert date here-->",date)
		.replace("<!--insert r number here-->",r)
		.replace("<!--insert credit here-->",credit)
		.replace("<!--insert width here-->",dimensions.width)
		.replace("<!--insert height here-->",dimensions.height);
	return {js:js, file:date+"_"+r};
};
