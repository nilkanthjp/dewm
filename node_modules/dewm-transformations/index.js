...
transformations.default = function(xml) {
	return xml.content
...
transformations.router = function(stack) {
	var routes = {
		...
		"fiction":transformations.dept,
		"crit":transformations.dept,
...
transformations.dept = function(xml,callback) {
	...
	utils.mongo.find("assignments",{stack:xml.stackname},{},function(err, items){
		var assignments = items[0],
			...
			content = transformations.default(xml),
			content = build.elements.insert(content,elements);
		callback(toolbox);
	});
};

var transformDept = function(theCondeNetXMLText,stackName,template,danville,rootDirectory,callback) {
	if (stackName.indexOf("briefly") > 0) {
		...
		$("#layoutSubmit").click(function() {
			...
			if (rootDirectory.nativePath.indexOf("TNYTablet") == -1) {
				danvilleHTML = danvilleHTML.replace(new RegExp("<!--insert caption here-->", 'g'), '<br><br><br><p class="caption" style="font-family:CaslonPro; font-size:1.25em; font-style:italic">'+danville.Caption+'</p><p class="credit" style="margin-top: .5em">'+danville.Credit+'</p><br><br><br>');	
			}
